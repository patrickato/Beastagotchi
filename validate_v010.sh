...
OUT=/tmp/beast-v010-validation
rm -rf "$OUT"; mkdir -p "$OUT"; chown pi:pi "$OUT"

echo '[1/10] core health/state/catalogs'
...
echo '[2/10] exact Jayofelony native frame source'
...
echo '[3/10] production touch decision'
...
echo '[4/10] installed Python compile/import gate'
...
assert beastcore.__version__ == '0.10.0'
assert beastui.__version__ == '0.10.0'
PY

echo '[5/10] all theme smoke renders'
...
echo '[6/10] Visualization Studio + non-Matrix theme variants'
sudo -u pi PYTHONPATH=/opt/beast-ui /opt/.pwn/bin/python3 "$SRC/tools/render_v010_validation.py" --root /opt/beast-ui --state "$OUT/state.json" --out "$OUT/previews"

echo '[7/10] renderer registry'
...
echo '[8/10] package unit regression'
...
echo '[9/10] services/runtime/journals'
{
...
echo '[10/10] package'
tar -czf /home/pi/beast-core-validation-v010.tar.gz -C /tmp beast-v010-validation
chown pi:pi /home/pi/beast-core-validation-v010.tar.gz
echo 'Validation complete: /home/pi/beast-core-validation-v010.tar.gz'
