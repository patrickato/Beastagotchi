# Beastagotchi Foundation Roadmap v1.3
...
- [x] Exact Pwnagotchi 2.9.5.9 epoch callback observed through Beast Bridge
- [ ] Capture GPS TPV/SKY with a real fix and motion sample (opportunistic)
...
- [x] v0.3 live validation on target Pi
...
- [ ] validate off-screen renderer under `/opt/.pwn` Python on target Pi
- [ ] establish exclusive physical display ownership safely
- [ ] validate live framebuffer rendering while Pwnagotchi engine remains active
...
Install Beast Core v0.4 and the Beast UI skeleton, validate the UI **off-screen only**, and return the generated validation archive. The physical framebuffer service remains disabled until display ownership is made reversible and conflict-free.
