...
- [ ] full Apps launcher
- [ ] notification center
- [~] user-editable page/widget layout studio — v0.15 spatial Dashboard/Board composer; built-in page widgets remain
...
- [~] Dashboard/Boards now support show/data/renderer/move/resize/z-order; arbitrary editing of built-in page widgets remains
...
- [~] arbitrary user-created pages through Beast Studio — named live Boards are dynamic App destinations; additional structured widget/page types remain.
...
- [x] Live Dashboard binds a variable user-selected set of canonical telemetry sources with real histories where applicable.
...
- [x] spatial live Dashboard/Board data binding + renderer selection + labels + move/resize/show/hide/z-order.
...
- [~] drag/resize/show/hide implemented for Dashboard and user Boards; built-in page internals remain.
...
## v0.15 anti-forgetting delta
- [x] direct spatial composer overlay uses the exact TFT grid/model
- [x] named custom Boards become Apps without increasing the fixed carousel
- [x] Context Decks can contain user Boards
- [ ] alignment guides / grouping / layer locks / multi-select
- [ ] structured list/map/plugin widgets in custom Boards
- [ ] Try On Device timed rollback from Studio

## v0.15.1 continuity audit delta
- [x] Re-reviewed original Design & Architecture Bible, early idea transcripts, visual concept/contact sheets, source audits, hardware/plugin specs and roadmap lineage before continuing.
- [x] Confirmed reserved identities/experiences remain explicit: Sonar/Submarine, Mission Control, Oscilloscope Lab, recent-SSID Matrix ambient mode, theme-reactive sound, theme-synchronized LEDs, dock transitions, per-session mood/theme history and visual Expedition replay.
- [x] v0.15.1 validator corrected for true module identity and Beast privileged action group.
- [x] Beast Studio HTTP service permits immediate safe local rebinding after validation stop/start cycles.
- [x] Validator waits for actual Studio API readiness and emits a diagnostic failure bundle instead of simply disappearing without an archive.
