...
class SceneRuntime:
    ...
    def layer(self, layer_id: str, kind: str, bounds,
              *, z: int = 0, signals: Iterable[str] = (),
              ...
              touch: str | None = None, reduced_motion: str = "static",
              decorative: bool = False):
        """Context manager that times and registers a semantic visual layer."""
        return _LayerContext(
            self,
            SceneLayerSpec(
                id=str(layer_id), kind=str(kind),
                bounds=tuple(int(v) for v in bounds), z=int(z),
                signals=tuple(str(x) for x in signals),
                update_class=str(update_class), cacheable=bool(cacheable),
                resource_class=str(resource_class), privacy=str(privacy),
                touch=None if touch is None else str(touch),
                reduced_motion=str(reduced_motion), decorative=bool(decorative),
            ),
        )


class _LayerContext:
    def __init__(self, runtime: SceneRuntime, spec: SceneLayerSpec) -> None:
    ...
    def __exit__(self, exc_type, exc, tb):
        self.runtime.register(self.spec, render_ms=(time.perf_counter() - self.started) * 1000.0)
        return False
