...
INTEGRATIONS: tuple[dict[str, Any], ...] = (
    ...
    {
        "id": "system-dbus", "label": "D-Bus System Provider Layer",
        "category": "platform", "lane": "os_packages", "maturity": "candidate",
        "provides": ["platform.dbus", "platform.event_driven_services"],
        "requires": ["package:python3-dbus-next"],
        "activation": "event_adapter", "isolation": "in_process_adapter",
        "purpose": "Async event-driven access to Linux service APIs without turning shell command parsing into a permanent integration contract.",
    },
    {
        "id": "network-networkmanager", "label": "NetworkManager Provider",
        "category": "connectivity", "lane": "os_service", "maturity": "candidate",
        "provides": ["network.manager", "network.connectivity_state", "network.captive_portal_state"],
        "requires": ["package:network-manager"],
        "optional_requirements": ["service:NetworkManager.service", "package:python3-networkmanager"],
        "activation": "provider_service", "isolation": "external_service",
        "purpose": "When the host uses NetworkManager, consume its D-Bus connectivity truth instead of duplicating connection-state logic.",
    },
    {
        "id": "audio-pipewire", "label": "PipeWire / WirePlumber Audio",
        "category": "media", "lane": "os_service", "maturity": "candidate",
        "provides": ["audio.output", "audio.input", "audio.device_hotplug", "audio.bluetooth"],
        "requires": ["package:pipewire", "package:wireplumber"],
        "activation": "provider_service", "isolation": "external_service",
        "purpose": "Optional audio routing layer for reactions, voice, microphones and Bluetooth audio without a Beast-specific audio stack.",
    },
    {
        "id": "knowledge-kiwix", "label": "Kiwix / ZIM Offline Knowledge",
        "category": "knowledge", "lane": "external_process", "maturity": "candidate",
        "provides": ["knowledge.zim", "knowledge.offline_search", "knowledge.offline_http"],
        "requires": ["package:kiwix-tools", "executable:kiwix-serve"],
        "activation": "on_demand_subprocess", "isolation": "sandboxed_process",
        "purpose": "Serve/search large offline manuals and reference libraries through Field Library without inventing a document container format.",
    },
    {
        "id": "camera-picamera2", "label": "Picamera2 / libcamera",
        "category": "camera", "lane": "os_packages", "maturity": "candidate",
        "provides": ["camera.capture", "camera.preview", "capsule.camera_source", "memory.visual_capture"],
        "requires": ["package:python3-picamera2"],
        "activation": "hardware_adapter", "isolation": "adapter_service_preferred",
        "purpose": "Optional camera provider for Capsule receive, explicitly enabled visual memories and future field imaging.",
    },
    {
        "id": "visual-lvgl", "label": "LVGL Alternate Display Runtime",
        "category": "visual", "lane": "external_process", "maturity": "experimental",
        "provides": ["visual.lvgl", "visual.gpu_capable", "display.large_screen_runtime"],
        "requires": ["package:libsdl2-dev"],
        "activation": "alternate_renderer", "isolation": "separate_presentation_process",
