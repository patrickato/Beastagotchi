...
def render(root,out,theme,state,page='home',setup=None,aux=None,events=None,physical=(480,320)):
    ui=BeastUI(root=root,output=str(out),theme_id=theme,physical_size=physical,display_mode='fit');ui.state=dict(state)
    ui.histories={
...
def main():
    ...
    # Compatibility-output proofs. These deliberately validate only the
    # display/touch boundary; they are not responsive-layout support claims.
    render(root,out/'compat-640x480.png','classic',state,page='overview',physical=(640,480))
    render(root,out/'compat-800x480.png','cyberpunk',state,page='dashboard',setup=spatial,physical=(800,480))
