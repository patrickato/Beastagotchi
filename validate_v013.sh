...
assert beastcore.__version__ == '0.13.0'
assert beastui.__version__ == '0.13.0'
assert beaststudio.__version__ == '0.13.0'
PY
...
for _ in $(seq 1 30); do systemctl is-active --quiet beast-studio.service && [[ -s /var/lib/beastagotchi/ui/studio.token ]] && break; sleep .25; done
TOKEN="$(cat /var/lib/beastagotchi/ui/studio.token)"
curl -fsS -H "X-Beast-Studio-Token: $TOKEN" http://127.0.0.1:8091/api/schema > "$OUT/studio-schema.json"
curl -fsS -H "X-Beast-Studio-Token: $TOKEN" http://127.0.0.1:8091/api/preferences > "$OUT/studio-preferences.json"
curl -fsS -H "X-Beast-Studio-Token: $TOKEN" http://127.0.0.1:8091/api/plugins > "$OUT/studio-plugins.json"
# Never archive the pairing token itself.

echo '[6/13] read-only plugin action planning through local socket'
sudo -u pi PYTHONPATH=/opt/beast-ui /opt/.pwn/bin/python3 - <<'PY' > "$OUT/action-plan.txt"
...
sudo -u pi PYTHONPATH=/opt/beast-ui /opt/.pwn/bin/python3 "$SRC/tools/render_v013_validation.py" --root /opt/beast-ui --state "$OUT/state.json" --events "$OUT/events.json" --out "$OUT/previews"

echo '[9/13] app/dashboard/runtime registry'
...
from beastui.customization import DEFAULT_DASHBOARD_WIDGETS,WIDGET_STYLES
print(json.dumps({'main_pages':Pages.IDS,'app_count':len(AppRegistry().all()),'categories':AppRegistry().categories(),'widget_styles':WIDGET_STYLES,'dashboard_defaults':DEFAULT_DASHBOARD_WIDGETS},indent=2))
...
import json
s=json.load(open('/tmp/beast-v013-validation/state.json'))
...
echo '[13/13] package'
tar -czf /home/pi/beast-core-validation-v013.tar.gz -C /tmp beast-v013-validation
chown pi:pi /home/pi/beast-core-validation-v013.tar.gz
echo 'Validation complete: /home/pi/beast-core-validation-v013.tar.gz'
