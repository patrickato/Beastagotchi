...
async function loadPacks(){try{
  ...
  (updates.components||[]).forEach(r=>{let d=document.createElement('div');d.className='plugin';let top=document.createElement('div');top.className='pluginTop';let n=document.createElement('span');n.className='pluginName';n.textContent=`${r.label||r.id} · ${r.installed_version||'unknown'}`;let st=document.createElement('span');st.className='pill '+(r.compatibility==='compatible'?'on':'');st.textContent=String(r.last_result||'not_checked').toUpperCase();top.append(n,st);d.append(top);let meta=document.createElement('div');meta.className='mini';meta.textContent=`${r.kind||'component'} · ${r.source_type||'unknown'}${r.source?' · '+r.source:''}`;d.append(meta);let sel=document.createElement('select');['manual','notify','auto_stage','auto_install'].forEach(v=>option(sel,v,v.replace('_',' ').toUpperCase()));sel.value=saved[r.id]||r.policy||'notify';sel.onchange=()=>setUpdatePolicy(r.id,sel.value,sel);d.append(sel);if((r.blockers||[]).length){let b=document.createElement('div');b.className='status';b.style.color='var(--warn)';b.textContent=(r.blockers||[]).join(' · ');d.append(b)}up.append(d)});
  let gate=document.createElement('div');gate.className='status';gate.textContent=`Auto trigger: ${updates.auto_trigger_ready?'ready':'not ready'} · dock ${updates.dock_ready?'yes':'no'} · internet ${updates.internet_state||'unknown'} · executor ${updates.executor_enabled?'enabled':'locked'}`;up.append(gate);
