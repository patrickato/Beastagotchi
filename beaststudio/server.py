...
<main><aside><div class="nav"><button class="tab active" data-tab="visual">VISUAL</button><button class="tab" data-tab="dashboard">DASH</button><button class="tab" data-tab="decks">DECKS</button><button class="tab" data-tab="data">DATA</button><button class="tab" data-tab="plugins">PLUGINS</button><button class="tab" data-tab="packs">PACKS</button><button class="tab" data-tab="global">GLOBAL</button><button class="tab" data-tab="search">SEARCH</button><button class="tab" data-tab="ops">OPS</button></div>
...
(async()=>{try{schema=await jfetch('/api/schema');base=await jfetch('/api/preferences');draft=clone(base);rebuild();await loadVariants();await loadGlobalProfile();$('conn').textContent='LIVE'}catch(e){$('conn').textContent='ERROR';$('status').textContent=e}})();
</script></body></html>
'''
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            if path in {'/api/plugin-plan','/api/plugin-toggle','/api/service-plan','/api/service-restart','/api/backup-plan','/api/backup-create','/api/support-plan','/api/support-create','/api/variant-save','/api/variant-load','/api/variant-delete','/api/update-policy','/api/pack-inspect','/api/pack-stage','/api/pack-install-plan','/api/pack-install','/api/pack-rollback-plan','/api/pack-rollback','/api/pack-activate-plan','/api/pack-activate','/api/pack-deactivate-plan','/api/pack-deactivate','/api/update-stage-plan','/api/update-stage','/api/update-pack-plan','/api/update-pack-apply','/api/experience-draft','/api/global-policy','/api/presentation-plan'} and not self._auth():return self._send(403,{'error':'paired Studio token required'})
