...
async function previewBuiltinExperience(id,page,btn){btn.disabled=true;try{
  let r=await fetch('/api/experience-preview',{method:'POST',headers:{'X-Beast-Studio-Token':TOKEN,'content-type':'application/json'},body:JSON.stringify({id,page})});
  ...
  $('previewModeLabel').textContent='Experience DNA renderer';$('previewMeta').textContent=id.toUpperCase()+' · '+page.toUpperCase()+' · Core read-only plan';
  $('status').textContent='Previewing '+id+' / '+page+'. Nothing was applied and TFT ownership remains unchanged.';
}catch(e){$('status').textContent='Built-in Experience preview failed: '+e}finally{btn.disabled=false}}
...
function renderBuiltinExperiences(plan){
  ...
  rows.forEach(r=>{let d=document.createElement('div');d.className='plugin';let cov=r.page_coverage||{},variant=r.variant||{},caps=r.capabilities||{},ready=!!r.ready_for_preview;
    ...
    let meta=document.createElement('div');meta.className='mini';meta.textContent=(variant.visual_family||'experience')+' · '+(variant.quality_variant||'full')+' · '+(variant.display_class||'unknown')+' display';d.append(meta);
    ...
    (cov.implemented||[]).forEach(page=>{let b=document.createElement('button');b.textContent='PREVIEW '+String(page).toUpperCase();b.onclick=()=>previewBuiltinExperience(r.experience_id,page,b);d.append(b)});
    box.append(d)});
