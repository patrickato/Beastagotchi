...
async function loadPacks(){try{
  ...
  let gate=document.createElement('div');gate.className='status';gate.textContent=`Metadata: ${updates.check_state||'--'} · ${updates.update_available_count||0} update(s) · trusted ${updates.trusted_component_count||0}/${updates.component_count||0} · auto trigger ${updates.auto_trigger_ready?'ready':'not ready'} · download ${updates.download_executor_enabled?'enabled':'locked'} · install ${updates.executor_enabled?'enabled':'locked'}`;up.append(gate);
