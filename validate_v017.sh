...
fail_bundle(){
  ...
  tar -czf /home/pi/beast-core-validation-v017-failed.tar.gz -C /tmp beast-v017-validation
  chown pi:pi /home/pi/beast-core-validation-v017-failed.tar.gz
  echo 'Failure bundle: /home/pi/beast-core-validation-v017-failed.tar.gz'
}
trap 'rc=$?; if [[ $rc -ne 0 ]]; then fail_bundle; fi; cleanup; exit $rc' EXIT

echo '[1/18] semantic Core readiness / protected engine'
for _ in $(seq 1 90); do
  ...
  if curl -fsS --max-time 2 'http://127.0.0.1:8090/state?meta=0' > "$OUT/state-ready.json" 2>/dev/null; then
    if python3 - <<'PY'
import json
s=json.load(open('/tmp/beast-v017-validation/state-ready.json'))
...
python3 - <<'PY'
import json
h=json.load(open('/tmp/beast-v017-validation/health.json'))
assert h.get('state') in {'healthy','degraded'},h
PY

echo '[2/18] live platform / knowledge / Operator catalogs'
...
echo '[3/18] bridge/native/touch/display-conflict invariants'
...
echo '[4/18] installed compile/import/version gate'
...
assert beastcore.__version__=='0.17.0';assert beastui.__version__=='0.17.0';assert beaststudio.__version__=='0.17.0'
PY

echo '[5/18] local privileged channel invariants'
...
echo '[6/18] safe action plans / Operator boundary (no mutation)'
...
import json
x=json.load(open('/tmp/beast-v017-validation/operator-tools.json'))
...
echo '[7/18] Beast Studio paired API readiness'
...
echo '[8/18] Field Library extractor/search capability'
python3 - <<'PY' > "$OUT/field-library-capability.txt"
import json
s=json.load(open('/tmp/beast-v017-validation/state.json'))
...
echo '[9/18] recovery backup inspection (read-only if backup exists)'
python3 - <<'PY' > "$OUT/backup-inspection.txt"
import json,urllib.parse,urllib.request
s=json.load(open('/tmp/beast-v017-validation/state.json'));items=s.get('backups.items') or []
if not items:
...
echo '[10/18] all-theme 480x320 off-screen smoke renders'
...
echo '[11/18] v0.17 validation gallery + compatibility proofs'
sudo -u pi PYTHONPATH=/opt/beast-ui /opt/.pwn/bin/python3 "$SRC/tools/render_v017_validation.py" --root /opt/beast-ui --state "$OUT/state.json" --events "$OUT/events.json" --out "$OUT/previews"

echo '[12/18] gallery dimensions / nonblank / compatibility gate'
/opt/.pwn/bin/python3 - <<'PY' > "$OUT/gallery-gate.txt"
from pathlib import Path
from PIL import Image,ImageStat
root=Path('/tmp/beast-v017-validation/previews');rows=[]
...
echo '[13/18] app/capability registry'
...
echo '[14/18] optional capability snapshot'
...
echo '[15/18] package unit regression'
...
echo '[16/18] service/runtime/performance evidence'
...
echo '[17/18] journals / support evidence'
...
echo '[18/18] package validation archive'
tar -czf /home/pi/beast-core-validation-v017.tar.gz -C /tmp beast-v017-validation
chown pi:pi /home/pi/beast-core-validation-v017.tar.gz
echo 'Validation complete: /home/pi/beast-core-validation-v017.tar.gz'
